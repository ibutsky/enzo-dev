...
/* Member functions for dealing with Cold Gas Subgrid Model */
  int CalculateColdGasDensityAndRadius(float *cold_cloud_density, float *cold_cloud_radius);
  int CalculateColdGasDragCoefficient(float *drag_coef);
...
/* Baryons: compute the cooling time. */
  ...
  int ComputeCoolingTimeAtSpecifiedTemperature(float *cooling_time,
		      float CoolingTemperature, int CoolingTimeOnly=FALSE, int ReturnAbsValue=TRUE);
...
/* Gravity Test (Sphere): check results. */
  ...
  int ThermalInstabilityInitializeGrid(float TIMeanDensity, float TIMeanTemperature,

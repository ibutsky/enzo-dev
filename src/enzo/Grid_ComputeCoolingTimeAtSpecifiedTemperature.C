...
/  date:       July 2020
/
/  PURPOSE: Computes the cooling throughout the grid for a user-specified
/           uniform temperature. Nearly identical to Grid_ComputeCoolingTime, 
/           only it defines gasenergy and totalenergy based on the parameter
/           CoolingTemperature.
/
/  RETURNS:
/
...
extern "C" void FORTRAN_NAME(cool_multi_time)(
	...
	float *cooltime, int *coolonly, int *returnabsvalue,
...
int grid::ComputeCoolingTimeAtSpecifiedTemperature(float *cooling_time, float CoolingTemperature,
						   int CoolingTimeOnly, int ReturnAbsValue)
{
  ...
  // fill out gasenergy and totalenergy fields
  float vx, vy, vz, v2;
  for(int n = 0; n < size; n++){
    ...
    totalenergy[n] = 0.5 * v2 + gasenergy[n];
  }
  ...
  if (grackle_data->use_grackle == TRUE) {
    ...
    if (HydroMethod==Zeus_Hydro) {
      thermal_energy = totalenergy;
    }
    else if (DualEnergyFormalism) {
      thermal_energy = gasenergy;
    }
    else {
      temp_thermal = TRUE;
      thermal_energy = new float[size];
      for (i = 0; i < size; i++) {
        thermal_energy[i] = totalenergy[i] - 
  ...
  if (MultiSpecies) {
    ...
    FORTRAN_NAME(cool_multi_time)(
       ...
       cooling_time, &CoolingTimeOnly, &ReturnAbsValue,
    ...
    FORTRAN_NAME(cool_time)(
       BaryonField[DensNum], totalenergy, gasenergy,
          BaryonField[Vel1Num], BaryonField[Vel2Num], BaryonField[Vel3Num],
       cooling_time, &ReturnAbsValue,
       GridDimension, GridDimension+1, GridDimension+2,
          &CoolData.NumberOfTemperatureBins, &ComovingCoordinates,
          &HydroMethod,

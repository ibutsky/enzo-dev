		      ...
		      int GravityOn, int NumberOfColours, int colnum[], float *pressure)
{
  ...
  for (i = 0; i < GridDimension[0]; i++) {
    ...
    if (ColdGasSubgridModel)
      for (k = 0; k < GridDimension[2]; k++) {
	index3 = (k*GridDimension[1] + j) * GridDimension[0] + i;
        cdslice[index2+k] = BaryonField[CDensNum][index3];
        cuslice[index2+k] = BaryonField[CVel1Num][index3];
        cvslice[index2+k] = BaryonField[CVel2Num][index3];
        cwslice[index2+k] = BaryonField[CVel3Num][index3];
      }
  ...
  float *dls, *drs, *flatten, *pbar, *pls, *prs, *ubar, *uls, *urs, *vls, 
    *vrs, *gels, *gers, *cdls, *cdrs, *wls, *wrs, *diffcoef, *df, *ef, *uf, *vf, *wf, *gef,
    *ges, *cdf, *colf, *colls, *colrs, *culs, *curs, *cuf,
  ...
  */
  ...
  for (i = 0; i < GridDimension[0]; i++) {
    ...
    if (ColdGasSubgridModel)
      for (k = 0; k < GridDimension[2]; k++) {
        index3 = (k*GridDimension[1] + j)*GridDimension[0] + i;
        BaryonField[CDensNum][index3] = cdslice[index2+k];
